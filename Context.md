...
- **Last updated:** 2026-07-11 21:47, checkpoint (**Session 04C item 2 (home-location text-search box) BUILT, ITERATED, AND SHIPPED TO PRODUCTION.** Search-as-you-jump box added to `step-homeloc`; switched from Nominatim to **Photon** (photon.komoot.io) after discovering Nominatim's `/search` can't autocomplete on partial words; hard-restricted results to ~40km around the map's current city via `bbox` after a soft `bounded=0` bias proved ineffective (confirmed via live testing); scoped to locality/city-level results only, per Nagesh's explicit call to drop landmark-level results to avoid implying only those spots are searchable. **Password strength rules also built**: 8+ chars/1 uppercase/1 special-char enforced both server-side (`src/routes/auth.js`, `/signup` + `PUT /password`) and client-side (live hints, match-indicator, full-field-required button gating, mandatory-field asterisks on the register form). Both features click-tested and confirmed working by Nagesh across several iteration rounds. **Committed (`107b8c8`) and promoted the full chain**: `feat/session-05-lead-scoring → uat` (fast-forward) `→ main` (merge commit `28c0e89`, `--no-ff`) — **live on production** at `https://mapit.co.in`, per Nagesh's explicit go-ahead ("Session 5 has passed all points"). This also carries Session 5 #2 (Claude-based lead scoring) to production in its safe fail-open (`unscreened`) state — still blocked on `ANTHROPIC_API_KEY`/Arun's sign-off, expected tomorrow. Next up: UAT checklist HTML persistence bug (`docs/session-04-uat-checklist-Arun.html`).)

---

## 🎯 Current Goal

**Next up: UAT checklist HTML persistence bug (`docs/session-04-uat-checklist-Arun.html`) — Arun reported pass/fail marks and comments reset on reopen; not yet scoped (2026-07-11).** This is a bug in the standalone checklist document's own client-side save logic, not MapIt application code. Also still pending: Arun's sign-off + `ANTHROPIC_API_KEY` creation for Session 5 #2 real classification, and Arun's budget-memo decisions (expected 2026-07-12, "tomorrow" per Nagesh).
...
| UAT checklist HTML persistence bug | 🔴 Not started (queued next) | `docs/session-04-uat-checklist-Arun.html` | Pass/fail marks + comments reset on reopen; standalone doc's own client-side save logic, not app code |
...
**⬅ Immediate:**
1. **Scope + fix the UAT checklist HTML persistence bug** (`docs/session-04-uat-checklist-Arun.html`) — Arun reported pass/fail marks and comments reset on reopen; standalone doc's own client-side save logic, not app code; **queued next per Nagesh (2026-07-11)**
2. **Arun's budget-memo decisions** — expected 2026-07-12 ("tomorrow" per Nagesh); 6 items across Sessions 5–9 (Anthropic key, Vercel/Resend/Supabase Pro upgrades, Twilio SMS OTP + DLT)
3. **Get Arun's sign-off + create `ANTHROPIC_API_KEY`** — console.anthropic.com/settings/keys; add to local `.env` first for a real genuine/spam test, then to Vercel (Production + Preview) — code is already live on production in its safe fail-open (`unscreened`) state
4. **Test real classification** — with the key in place, send one genuine-sounding note and one spam-like note, confirm `lead_verdict` differs correctly and email wording matches
5. **If/when Arun approves the plan upgrades** — schedule Vercel Pro before Session 7, Resend Starter + Supabase Pro before Session 9
